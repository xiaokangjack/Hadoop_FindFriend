...
public class DeltaDistanceMapper extends
	...
	public void setup(Context cxt) throws IOException{
		densityPath = new Path(cxt.getConfiguration().get("DENSITYPATH"));//获取局部密度文件的存储目录
			...
			try {
				...
				while (reader.next(dkey, dvalue)) {// 循环读取文件
					...
					if(dvalue.get()>max_density){// 寻找最大局部密度的下标
						max_density=dvalue.get();//保存当前最大局部密度
						max_density_vector_id=dkey.get();//保存当前最大局部密度的下标
					}
				}
			} catch (Exception e) {
				e.printStackTrace();
